    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const dataTp = await ticDocsdiscountService.getDiscounttpLista(props.item.docs);
                ...
                console.log("QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ", props.item)
    ...
    const onDropdownChange = (e, item) => {
        const updatedTicDocsdiscounts = ticDocsdiscounts.map((docsItem) => {
            if (docsItem.id === item.id) {
                ...
                let _popust;
                if (foundItem.condition && foundItem.condition.endsWith('%')) {
                    docsItem.procenat = foundItem.condition.slice(0, -1)
                    _popust = foundItem.condition.slice(0, -1) * 0.01 * docsItem.price * docsItem.output;
                } else {
                    docsItem.procenat = 0
                    _popust = foundItem.condition;
                }
