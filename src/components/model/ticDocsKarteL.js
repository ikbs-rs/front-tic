...
export default function TicTransactionsL(props) {
    ...
    const valueTemplate = (rowData, name, e) => {
        ...
        if (name == 'tickettp') {
            if (rowData?.id) {
                const dataDD = ddTickettpItems[rowData.id];
                // console.log(dataDD, "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW", ddTickettpItems)
                const ddTickettpItem = dataDD.map(({ text, id }) => ({ name: text, code: id }));

                if (ddTickettpItem) {
                    const dropdownValue = ddTickettpItem?.find((item) => item.code == rowData.tickettp);
                    if (dropdownValue) {
                        return (
